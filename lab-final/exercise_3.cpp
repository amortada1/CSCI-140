/* 
    Program: Large Integers V3
    Author: Ali Mortada
    Class: CSCI 140
    Date: 6/6/23
    Description: Same as Large Integers V2 from PA 5, but can add a large integer with a single digit.
...
void addDigit(const vector<int>, int, vector<int>&);
...
int main() {
    ...
    // If input 2 is a single digit, convert it into an int, add numbers, then exit program
    if (input2.size() == 1) {
        digit = input2.at(0) - '0';
        stringToVector(input1, num1);
        addDigit(num1, digit, result);
        return 0;
    }
...
// Addition for a large integer with a single digit
void addDigit(const vector<int> op1, int digit, vector<int> &result) {
    int carry;
    // Add digit and first digit of large int
    result.at(0) = digit + op1.at(0);
    // If result is 10 or greater, mod it and carry the 1
    if (result.at(0) >= 10) {
        result.at(0) %= 10;
        carry = 1;
        // Iterate through rest of vector
        for (int i = 1; i < MAX_DIGITS; i++) {
            result.at(i) = op1.at(i) + carry;
            // If result at point is greater than 10, carry the 1
            if (result.at(i) >= 10) {
                result.at(i) %= 10;
                result.at(i + 1) = 1;
                carry = 1;
    ...
    for (int i = MAX_DIGITS - 1; i >= 0; i--) {
        ...
        op1String.append(to_string(op1.at(i)));
    }
    resultString.erase(0, resultString.find_first_not_of('0'));
    op1String.erase(0, op1String.find_first_not_of('0'));
    cout << op1String << " + " << digit << " = " << resultString;
}
